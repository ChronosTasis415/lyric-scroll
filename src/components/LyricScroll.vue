<template>
  ...
  >
    ...
    >
      ...
      >
        <p :style="{lineHeight: lyricLineheight}" @click="onJumpClick(item[0])">{{item[1]}}</p>
        <!-- 翻译 -->
    ...
    <div class="center-mark" v-if="isDragging">
      <div
        class="triangle"
        :style="{
          borderColor: `transparent transparent transparent rgba(0, 0, 0, 0.16)`,
...
export default {
  ...
  computed: {
    ...
    lyricTopPadding() {
      ...
      return this.firstLyricHeight / 2;
    },
  ...
  methods: {
    ...
    onJumpClick(time) {
      this.$emit('change-current-time', time)
      this.$nextTick(() => {
        this.isDragging = false
      })
    },
    ...
    updateLyricPos() {
      ...
      if (!this.isDragging) {
        ...
        const scrollTarget = this.offsetTopList[activeLyricIdx] + this.offsetHeightList[activeLyricIdx] / 2 - this.firstLyricHeight / 2;
    ...
    // 找出当前视野中最接近中间的歌词，返回下标
    findCenterLyricIdx() {
      const halfViewHeight = this.viewHeight / 4;
